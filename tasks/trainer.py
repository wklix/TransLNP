...
class Trainer(object):
    ...
    def decorate_torch_batch(self, batch):
        ...
        """
        net_input, net_target,weight = batch
        if isinstance(net_input, dict):
            net_input, net_target,weight = {
                k: v.to(self.device) for k, v in net_input.items()}, net_target.to(self.device),weight.to(self.device)
        else:
            net_input, net_target,weight = {'net_input': net_input.to(
                self.device)}, net_target.to(self.device),weight.to(self.device)
        if self.task == 'repr':
            net_target = None
            ...
            net_target = net_target.float()
            weight= weight.float()
        return net_input, net_target, weight
